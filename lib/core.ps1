...
function Get-UserAgent() {
    return "Scoop/1.0 (+http://codescoop.sh/) PowerShell/$($PSVersionTable.PSVersion.Major).$($PSVersionTable.PSVersion.Minor) (Windows NT $([System.Environment]::OSVersion.Version.Major).$([System.Environment]::OSVersion.Version.Minor); $(if(${env:ProgramFiles(Arm)}){'ARM64; '}elseif($env:PROCESSOR_ARCHITECTURE -eq 'AMD64'){'Win64; x64; '})$(if($env:PROCESSOR_ARCHITEW6432 -in 'AMD64','ARM64'){'WOW64; '})$PSEdition)"
}
...
function installed_apps($global) {
    $dir = appsdir $global
    if (Test-Path $dir) {
        Get-ChildItem $dir | Where-Object { $_.psiscontainer -and $_.name -ne 'codescoop' } | ForEach-Object { $_.name }
    }
}
...
function shim($path, $global, $name, $arg) {
    ...
    if ($path -match '\.(exe|com)$') {
        ...
        if (!(Get-CommandPath git)) {
            ...
            error "Please install git ('codescoop install git') and try again."
            exit 1
        }
...
function get_shim_path() {
    $shim_path = "$(versiondir 'codescoop' 'current')\supporting\shims\kiennq\shim.exe"
    $shim_version = get_config SHIM 'default'
    switch ($shim_version) {
        '71' { $shim_path = "$(versiondir 'codescoop' 'current')\supporting\shims\71\shim.exe"; Break }
        'scoopcs' { $shim_path = "$(versiondir 'codescoop' 'current')\supporting\shimexe\bin\shim.exe"; Break }
...
function Confirm-InstallationStatus {
    ...
    $Installed = @()
    $Apps | Select-Object -Unique | Where-Object { $_ -ne 'codescoop' } | ForEach-Object {
...
function Test-ScoopCoreOnHold() {
    ...
    if ([System.DateTime]::TryParse($hold_update_until, $null, [System.Globalization.DateTimeStyles]::AssumeLocal, [ref]$parsed_date)) {
        if ((New-TimeSpan $parsed_date).TotalSeconds -lt 0) {
            ...
            warn "If you want to update Scoop Core immediately, use 'codescoop unhold codescoop; codescoop update'."
            return $true
        } else {
        ...
        error "use 'codescoop hold codescoop' or 'codescoop config hold_update_until <YYYY-MM-DD>/<YYYY/MM/DD>'."
    }
...
$configFile = "$configHome\codescoop\config.json"
...
if ($scoopConfig) {
    ...
    if ($scoopConfigChg) {
        # Only save config file if there was a change
        ConvertTo-Json $scoopConfig | Out-UTF8File -FilePath $configFile
    }
}
# END NOTE
...
$scoopdir = $env:SCOOP, (get_config ROOT_PATH), "$([System.Environment]::GetFolderPath('UserProfile'))\codescoop" | Where-Object { -not [String]::IsNullOrEmpty($_) } | Select-Object -First 1

# Scoop global apps directory
$globaldir = $env:SCOOP_GLOBAL, (get_config GLOBAL_PATH), "$([System.Environment]::GetFolderPath('CommonApplicationData'))\codescoop" | Where-Object { -not [String]::IsNullOrEmpty($_) } | Select-Object -First 1
